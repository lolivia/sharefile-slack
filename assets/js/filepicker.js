...
(function (exports) {
    ...
    var initi$electors = function (options) {
		...
		if(!selectors.sharebutton)  selectors.sharebutton= "#btnClick1";
		if(!selectors.returnbutton)  selectors.returnbutton= "#btnClick2";
		...
		$("body").on("click", "a", function(e) {
			...
			if($this.hasClass("cancel")){
				...
				if ($selectedItem.length > 0) {
					filename_copy = $selectedItem.parents("li:first").children("span.name").find("a")[0].innerText;//Filename
    ...
    var onQuerySuccess = function (data) {
        ...
        if (data.DownloadToken) {
            if (copyDownloadLink.indexOf(arguments[1]) > -1) {
                promptUser(data.DownloadUrl,filename_copy,slackurl);
    ...
    var promptUser = function (text,filename_copy,slackurl) {
        window.prompt("Copy to clipboard: Ctrl+C, Enter", text);
		
		var url = "/inwebhook"+"?copyurl="+text+"&filename_copy="+filename_copy+"&"+slackurl;
	...
	var onclick1 = function()
	{
		...
		var $selectedItem = $contentsSection.find("input[type=checkbox]:checked"); // locate the first selected item
    if($selectedItem.length==0)
    {alert("Please select the files")}
    else {

      $($selectedItem[0]).parents("li:first").find("span.name a.filename")[0].innerText
      if ($selectedItem.length > 0) {
        var i = 0;
        for (i = 0; i < $selectedItem.length; i++) {
          var $Item = $($selectedItem[i]);
          filename_copy = $Item.parents("li:first").children("span.name").find("a")[0].innerText;//Filename

          var fdata = $Item.parents("li:first").data(); // fetch associated data from item
          if (fdata.type == "folder") {
            alert("Can not share folder");
            break;
          }
          var url = "/sharefile/getsharelink/" + "?itemid=" + fdata.id + "&access_token=" + access_token + "&subdomain=" + subdomain + "&appcp=" + appcp + "&title=" + filename_copy;
          var sharelink = null;
          // ajax call to ShareFile Rest APIs
          var fileinfo = $.ajax({
            type: "Get",
            ...
            complete: function (data) {
              console.log(data)
              sharelink = data.responseJSON.body.Uri;
              sendtoslack(sharelink, data.responseJSON.body.Title);
              /*if( sharelink != null )
               {
               var url = "/inwebhook"+"?copyurl="+sharelink+"&filename_copy="+filename_copy+"&"+slackurl;
        ...
        ep.after('completeshare', $selectedItem.length, function (data) {
            alert("Complete file share, go to Slack");
	...
	function sendtoslack(sharelink,filename_copy)
	{
		if( sharelink != null )
		{
			var url = "/inwebhook"+"?copyurl="+sharelink+"&filename_copy="+filename_copy+"&"+slackurl;
